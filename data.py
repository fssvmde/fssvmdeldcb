...
class VCDecSingleDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir):
        self.mel_dir = os.path.join(data_dir, 'mels')
        mels = os.listdir(self.mel_dir)
        self.test_ids = mels[:25]
        self.valid_ids = mels[25:30]
        self.train_ids = mels[30:]
        print("Total number of training wavs is %d." % len(self.train_ids))
        random.seed(random_seed)
        random.shuffle(self.train_ids)

    def get_mel(self, mel_id):
        mel_path = os.path.join(self.mel_dir, mel_id)
        mel = np.load(mel_path)
        mel = torch.from_numpy(mel).float()
        return mel

    def __getitem__(self, index):
        mel = self.get_mel(self.train_ids[index])
        item = {'mel': mel}
        return item
    ...
    def get_valid_dataset(self):
        ...
        for i in range(len(self.valid_ids)):
            mel = self.get_mel(self.valid_ids[i])
            mels.append(mel)
        return mels
    ...
    def get_test_dataset(self):
        ...
        for i in range(len(self.test_ids)):
            mel = self.get_mel(self.test_ids[i])
            mels.append(mel)
        return mels


class VCDecSingleBatchCollate(object):
    def __call__(self, batch):
        B = len(batch)
        mels = torch.zeros((B, n_mels, train_frames), dtype=torch.float32)
        ...
        for i, item in enumerate(batch):
            ...
            if mel.shape[-1] < train_frames:
                ...
                mel_length = train_frames
            mels[i, :, :mel_length] = mel[:, starts[i]:starts[i] + mel_length]
            mel_lengths.append(mel_length)
        mel_lengths = torch.LongTensor(mel_lengths)
        return {'mel': mels, 'mel_lengths': mel_lengths}


class VCDecMultiDataset(torch.utils.data.Dataset):
...
class VCDecMultiBatchCollate(object):
    def __call__(self, batch):
        B = len(batch)
