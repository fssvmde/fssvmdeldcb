...
import params
from data import VCDecSingleDataset, VCDecSingleBatchCollate
from model.vc import DiffVCSingle
...
if __name__ == "__main__":
    ...
    train_set = VCDecSingleDataset(data_dir)
    collate_fn = VCDecSingleBatchCollate()
    ...
    model = DiffVCSingle(n_mels, channels, filters, heads, layers, kernel, 
                         dropout, window_size, enc_dim, dec_dim, beta_min, 
                         beta_max).cuda()
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in tqdm(train_loader, total=len(train_set)//batch_size):
            mel, mel_lengths = batch['mel'].cuda(), batch['mel_lengths'].cuda()
            model.zero_grad()
        ...
        print(msg)
        with open(f'{log_dir}/train_dec_single.log', 'a') as f:
            f.write(msg)
        losses = []
        ...
        with torch.no_grad():
            ...
            for i, mel in enumerate(mels):
                ...
                mel_avg, mel_rec = model(mel, mel_lengths, n_timesteps=100)
                if epoch == save_every:
        ...
        torch.save(ckpt, f=f"{log_dir}/vc_single_{epoch}.pt")
